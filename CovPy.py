...
# this colors may cause error on windows
# ===== Cool Fontcolors =====
class fontcolor():
	def green(string):
...
def global_stats():
	...
	try:
		print(symbol.INFO + fontcolor.white('   Active Cases :'), "{:,}".format(covid.get_total_active_cases()))
		print(symbol.INFO + fontcolor.white('Confirmed Cases :'), "{:,}".format(covid.get_total_confirmed_cases()))
		print(symbol.INFO + fontcolor.white('Recovered Cases :'), "{:,}".format(covid.get_total_recovered()))
		print(symbol.INFO + fontcolor.white('    Death Cases :'), "{:,}".format(covid.get_total_deaths()))
	except Exception:
...
def display_name_id():
	...
	try:
		...
		for country in countries:
		  ...
		  country_id_table.add_row([fontcolor.grey('-')*5, fontcolor.grey('-')*32])
...
def status_with_name():
	...
	country_name_data_table = PrettyTable([fontcolor.cyan('Parameter'),fontcolor.cyan('Value')])
	try:
		country_name = input(symbol.INPUT + fontcolor.cyan("Enter Country Name : ")).strip()
		...
		for k, v in country_name_stats.items():
			...
			country_name_data_table.add_row([fontcolor.grey('-')*15,fontcolor.grey('-')*15])
		print("\n")
		print(country_name_data_table)
	except:
		print(symbol.WARN + fontcolor.red("Wrong Country Name Or No Internet Connection!"))
	finally:
		continue_msg()
...
def status_with_id():
	...
	country_id_data_table = PrettyTable([fontcolor.cyan('Parameter'),fontcolor.cyan('Value')])
	try:
		...
		for k, v in country_id_stats.items():
			country_id_data_table.add_row([fontcolor.white(k).capitalize(),fontcolor.white(v)])
			country_id_data_table.add_row([fontcolor.grey('-')*15, fontcolor.grey('-')*15])
		print("\n")
		print(country_id_data_table)
	except:
		print(symbol.WARN + fontcolor.red("Wrong Country ID Or No Internet Connection!"))
	finally:
		continue_msg()
